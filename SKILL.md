...
4) Leer [estilo de escritura](references/writing-style.md) para el flujo de trabajo de Word: estilos de fuente/párrafo por nivel (Book/Title/Chapter/Topic/Article Number/Article Title/Articulado), saltos de página (solo antes de cada Libro en estructura jerárquica) y la Tabla de Contenido nativa de Word (único mecanismo de navegación del documento).
...
8) Redacta el Título (no explicativo) y la fórmula de sanción con el ente que el usuario indique; si no, usa marcador genérico.
...
13) Aplica estructura de navegación y salto de página (detalle técnico en [estilo de escritura](references/writing-style.md)):
   - Salto de página entre “Proyecto de Ley”, “Fundamentos” y “Anexos” (si aplica).
   - Si el proyecto usa jerarquía (Orgánica >100 artículos): salto de página adicional antes de cada nuevo Libro (Título, Capítulo, Sección y Artículo no llevan salto de página propio).
   - Aplica los estilos de fuente y párrafo definidos en [estilo de escritura](references/writing-style.md) a cada nivel (Book/Title/Chapter/Topic/Article Number/Article Title/Articulado) — no usar los estilos de encabezado por defecto de Word, que fuerzan un color de tema en vez de "Automatic". Fundamentos y Anexo usan exactamente el estilo Chapter (encabezado) y Articulado (cuerpo), igual que Capítulo.
...
- Estructura con **salto de página** entre secciones:
  0) Contenido — Tabla de Contenido nativa de Word (Automático 1, Niveles 1-3: Libro/Título/Capítulo, más Fundamentos/Anexo)
  1) Proyecto de Ley (salto de página adicional antes de cada Libro si aplica jerarquía; Título no lo lleva)
  2) Fundamentos (mismo estilo Chapter/Articulado que Capítulo/Anexo)
  3) Anexos (si se aplica; mismo estilo Chapter/Articulado que Fundamentos)
